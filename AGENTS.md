...
**Now:** no open slice (see `docs/STATUS.md`). Next named work is write Config MCP (plan 18, not written). Deploy is parked — no custom domain yet. PrimeVue stays out. Do not write or implement Deploy. Do not start OAuth. Do not start room MCP. Do not start Config MCP. Do not drop remote D1 unless José asks. Do not start a slice that STATUS does not name.
