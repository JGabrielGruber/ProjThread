...
**Now:** operator-CRUD plan (see `docs/STATUS.md`). Deploy is parked — no custom domain yet. PrimeVue stays out. Do not write or implement Deploy. Do not start OAuth. Do not start room MCP. Do not start empty-tenant or Config MCP. Do not start a slice that STATUS does not name. Execute 16 only.
