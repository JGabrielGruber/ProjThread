...
**Now:** no open slice (see `docs/STATUS.md`). Next named work is operator CRUD (plan 16 not written). Deploy is parked — no custom domain yet. PrimeVue stays out. Do not write or implement Deploy. Do not start OAuth. Do not start room MCP. Do not start operator CRUD, empty-tenant, or Config MCP. Do not start a slice that STATUS does not name.
