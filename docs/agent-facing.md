...
**Operators** (Grok Bot, worker MCP): `.grok/skills/using-projthread/SKILL.md`. **Specialist** (repo + dogfood / empty workspace): `.grok/skills/projthread-specialist/SKILL.md`.

Still wrap catalog/wiki HTTP. Do not re-add wrap names (`me`, `list_*`, `get_node`, …). Do not add room tools. Node markdown stays in `content[0]`; envelope `content[1]` must not repeat `node.content`. Blob bytes are HTTP `GET /api/nodes/:id/blob` (Bearer), not `/mcp`. PUT is 405. Create over account quota is 507 `{ error: "quota" }`. Envelope keeps `mime_type` / `byte_size` / `filename` and strips `blob_key`. `wiki_create` is markdown|json, not blob. `attach_node_project` wraps `POST /api/nodes/:id/projects`.
